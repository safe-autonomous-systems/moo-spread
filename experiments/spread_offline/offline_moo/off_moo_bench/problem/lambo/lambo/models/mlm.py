...
def sample_tokens(base_tokens, logit_batch, enc_tokenizer, replacement=False, temp=1.0):
    logit_batch /= temp
...
def evaluate_windows(
    base_seqs, encoder, mask_size, replacement=True, encoder_obj="mlm"
):
    window_mask_idxs = {}
    window_entropy = {}
    window_features = {}
    ...
    for idx, seq in enumerate(base_seqs):
        ...
        for mask_start in range(offset, tokens.shape[-1] - 1, mask_size):
            ...
            with torch.no_grad():
                ...
                np.put_along_axis(
                    masked_inputs, mask_idxs, encoder.tokenizer.masking_idx, axis=1
                )
...
def mlm_train_epoch(model, optimizer, train_loader, mask_ratio):
    ...
    for minibatch in train_loader:
        ...
        loss, masked_logits, masked_tokens = mlm_train_step(
            model, optimizer, token_batch, mask_ratio
        )
...
def fit_masked_language_model(
    ...
    for epoch in range(num_epochs):
        metrics = {}
        metrics.update(mlm_train_epoch(model, optimizer, train_loader, mask_ratio))
