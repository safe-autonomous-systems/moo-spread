...
class MolGraph(object):
    ...
    def get_root_atoms(self):
        ...
        for idx in self.init_atoms:
            ...
            bad_neis = [
                y for y in atom.GetNeighbors() if y.GetIdx() not in self.init_atoms
            ]
            if len(bad_neis) > 0:
                roots.append(idx)
        return roots
    ...
    def get_bfs_order(self):
        ...
        for i, root in enumerate(self.root_atoms):
            self.mol_graph.nodes[root]["pos"] = (
                i + 1
            )  # interior atoms are 0, boundary atoms are 1,2,...
    ...
    def tensorize(mol_batch, avocab=common_atom_vocab):
        ...
        for i, hmol in enumerate(mol_batch):
            ...
            order = [
                (x + offset, add(y, offset), add_list(z, offset), t)
                for x, y, z, t in hmol.order
            ]
