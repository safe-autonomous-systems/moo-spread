...
def compute_metrics(obj_batch):
    ...
    print(
        "Pareto size : {}, Hypervolume : {:.0f}, Sparsity : {:.2f}".format(
            len(objs), hypervolume, sparsity
        )
    )
...
for i in range(len(sorted_ep_objs)):
    if sorted_population_objs != []:
        ...
        epoch_population_drawing.append(
            ax[0].scatter(
                cur_population_objs[0], cur_population_objs[1], s=15, color="grey"
            )
        )
    cur_ep_objs = np.transpose(sorted_ep_objs[i].copy())
    epoch_pareto_drawing.append(
        ax[0].scatter(cur_ep_objs[0], cur_ep_objs[1], s=15, color="black")
    )
    ...
    for j in range(N_elites):
        ...
        cur_weights_drawing.append(
            ax[0].arrow(
                cur_elites_objs[0][j],
                cur_elites_objs[1][j],
                sorted_elites_weights[i][j][0] * 30,
                sorted_elites_weights[i][j][1] * 30,
                width=0.1,
                head_width=3,
                head_length=5,
            )
        ...
        if have_pred:
            cur_predictions_drawing.append(
                ax[0].scatter(
                    cur_predictions[0][
                        j * N_prediction_step : (j + 1) * N_prediction_step
                    ],
                    cur_predictions[1][
                        j * N_prediction_step : (j + 1) * N_prediction_step
                    ],
        ...
        if have_offspring and i < len(sorted_ep_objs) - 1:
            cur_offsprings_drawing.append(
                ax[0].scatter(
                    cur_offsprings[0][(j + 1) * N_offsprings - 1],
                    cur_offsprings[1][(j + 1) * N_offsprings - 1],
                    s=15,
                    color="red",
                )
            )
    ...
    utopians_drawing.append(
        ax[0].scatter(utopians[i][0], utopians[i][1], s=80, marker=(5, 1), color="r")
    )
...
def press(event):
    ...
    if cur_elite == -1:
        ...
        sys.stdout.write(
            "\rcur = {}, cur_elite = {}, weight = {}, utopian = {}".format(
                cur, cur_elite, sorted_elites_weights[cur][cur_elite], utopians[cur]
            )
        )
    sys.stdout.flush()
