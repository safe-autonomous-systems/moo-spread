...
def on_click(event):
    if event.inaxes != ax:
        return
    if event.button != MouseButton.LEFT or not event.dblclick:
        return
    ...
    play_script = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "mujoco_play.py"
    )
    play_cmd = "python {} --env {} --model {} --env-params {}".format(
        play_script, args.env, model_file, env_params_file
    )
...
def compute_hypervolume_sparsity(obj_batch, ref_point):
    ...
    for i in range(len(objs)):
        hypervolume += (max(ref_point[0], objs[i][0]) - x) * (
            max(ref_point[1], objs[i][1]) - ref_point[1]
        )
...
parser.add_argument("--ref-point", type=float, nargs="+", default=[0.0, 0.0])
...
with open(args.log_path, "r") as fp:
    ...
    for j, line_data in enumerate(data):
        ...
        model_file_list.append(
            os.path.join(os.path.dirname(args.log_path), "EP_policy_" + str(j) + ".pt")
        )
        env_params_file = os.path.join(
            os.path.dirname(args.log_path), "EP_env_params_" + str(j) + ".pkl"
        )
