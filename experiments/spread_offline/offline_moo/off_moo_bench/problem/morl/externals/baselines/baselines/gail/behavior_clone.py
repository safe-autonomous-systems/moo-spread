...
def argsparser():
    ...
    boolean_flag(
        parser, "save_sample", default=False, help="save the trajectories or not"
    )
    parser.add_argument(
...
def learn(
    ...
    lossandgrad = U.function(
        [ob, ac, stochastic], [loss] + [U.flatgrad(loss, var_list)]
    )
...
def main(args):
    ...
    env = bench.Monitor(
        env, logger.get_dir() and osp.join(logger.get_dir(), "monitor.json")
    )
    env.seed(args.seed)
