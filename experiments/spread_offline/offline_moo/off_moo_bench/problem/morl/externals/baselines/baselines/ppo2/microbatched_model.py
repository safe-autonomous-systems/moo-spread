...
class MicrobatchedModel(Model):
    ...
    def __init__(
        ...
        *,
        ...
        ), "microbatch_size ({}) should divide nbatch_train ({}) evenly".format(
        ...
        self.grads_ph = [
            tf.placeholder(dtype=g.dtype, shape=g.shape) for g in self.grads
        ]
    ...
    def train(
        ...
        assert (
            states is None
        ), "microbatches with recurrent models are not supported yet"
        ...
        for microbatch_idx in range(self.nmicrobatches):
            _sli = range(
                microbatch_idx * self.microbatch_size,
                (microbatch_idx + 1) * self.microbatch_size,
            )
            td_map = {
        ...
        feed_dict = {
            ph: sum_g / self.nmicrobatches
            for ph, sum_g in zip(self.grads_ph, sum_grad_v)
        }
