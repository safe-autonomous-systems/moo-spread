...
def predict_hyperbolic(args, opt_graph, optgraph_id, test_weights):
    ...
    while True:
        objs_data, weights_data, delta_objs_data = collect_nearest_data(
            opt_graph, optgraph_id, threshold
        )
        cnt_data = 0
...
class Population:
    ...
    def prediction_guided_selection(
        ...
        for sample in self.sample_batch:
            ...
            angle_bound = [angle_center - np.pi / 4.0, angle_center + np.pi / 4.0]
            test_weights = []
            for i in range(num_weights):
                angle = (
                    angle_bound[0]
                    + (angle_bound[1] - angle_bound[0]) / (num_weights - 1) * i
                )
                weight = np.array([np.cos(angle), np.sin(angle)])
                if weight[0] >= -1e-7 and weight[1] >= -1e-7:
                    duplicated = False
                    for succ in opt_graph.succ[
                        sample.optgraph_id
                    ]:  # discard duplicate tasks
        ...
        for _ in range(N):
            ...
            virtual_ep_objs_batch = new_objs_batch[
                get_ep_indices(new_objs_batch)
            ].tolist()
