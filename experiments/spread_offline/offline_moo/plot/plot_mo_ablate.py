...
ax.bar(x, ranks, color=colors, width=0.6)

ax.set_ylabel("Avg. Rank")
ax.set_title("Average rank over 6 tasks")
...
df1 = pd.read_csv("1-mo-solver-ablate.csv").iloc[1:, 1:8]
df2 = pd.read_csv("2-mo-solver-ablate.csv").iloc[1:, 1:8]

rank1 = df1.rank(axis=0)
rank2 = df2.rank(axis=0)

mean_rank = (rank1 + rank2) / 2

combined_rank = rank2.combine_first(rank1)
mean_rank = (rank1 + combined_rank) / 2
...
data = example_data()
print(mean_rank.mean(axis=1))
data = mean_rank.fillna(4).to_numpy()

# fig, axs = plt.subplots(figsize=(7, 6), nrows=1, ncols=1,
...
ax.set_varlabels(["DTLZ5", "VLMOP2", "RE34", "ZDT2", "Molecule", "OmniTest", "MO-NAS"])
labels = ("NSGA-II", "NSGA-III", "MOEA/D", "MOBO")
ax.legend(labels, bbox_to_anchor=(0.94, 0), loc=3, borderaxespad=0)
# axs.legend(labels, loc='lower right')
plt.savefig("plot_test.png")
plt.savefig("mo-ablate.pdf")
