...
from off_ms_utils import ALLTASKSDICT
...
def mgd_armijo_step(
    ...
    for _ in range(max_backtracks):
        ...
        f_new = torch.stack(objective_functions(trial_x, classifiers=classifiers), 
                            dim=1)
...
def create_video_with_images(image_folder, video_filename):
    ...
    # image_folder = 'images/TF_ddmp_zdt3_v03'
    # Adjust the file pattern (e.g., '*.png' or '*.jpg') as needed
...
def get_non_domibated_points(
    points_pred, objective_functions, problem, keep_shape=True, classifiers=None, list_fi = None, indx_only = False
):
    pf_points = copy.deepcopy(points_pred.detach())
    if list_fi is None:
        with torch.no_grad():
            list_fi = objective_functions(pf_points, classifiers=classifiers)
    p_front = torch.stack(list_fi, dim=1).cpu().detach().numpy()
...
def select_top_n_candidates(
    ...
    Args:
        ...
        task_name: problem specification
        style: "hv" or "crowding" for selecting points based on hypervolume or crowding distance.
    ...
    if style == "crowding":
        full_list_fi = f(points, classifiers=classifiers)
        full_p_front = torch.stack(full_list_fi, dim=1)
...
def solve_for_h(
    ...
    g_targ = torch.randn((1, g.shape[1]), device=g.device)  # (1, d) # random target direction

    # Initialize h
    # h = g_val.clone().detach().requires_grad_()
    ...
    for step in range(num_inner_steps):
        ...
        F_vals = f(x_t_h, classifiers=classifiers)
...
def repair_bounds(x, xl, xu, args):
    ...
    lower = torch.tensor(xl, dtype=x.dtype, device=x.device).unsqueeze(
                0
            )  # Shape: [1, d]
    upper = torch.tensor(xu, dtype=x.dtype, device=x.device).unsqueeze(
                0
            )  # Shape: [1, d]
...
def plot_pareto_front(list_fi, t, args, extra = None, lab = None):
    ...
    if not os.path.exists(f"images_ms/"):
        os.makedirs(f"images_ms/")

    if lab is None:
        plt.savefig(
            f"images_ms/{name}.jpg",
            dpi=300,
            bbox_inches="tight",
        )
    else:
        plt.savefig(
            f"images_ms/{name}_{lab}.jpg",
            dpi=300,
            bbox_inches="tight",
        )