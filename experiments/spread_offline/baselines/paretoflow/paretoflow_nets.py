...
from baselines.paretoflow.paretoflow_utils import get_reference_directions
...
class FlowMatching(nn.Module):
    ...
    def get_N_non_dominated_solutions(cls, res_x, res_y, N, classifiers):
        if isinstance(res_x, np.ndarray):
            x_tensor = torch.tensor(res_x).float().to(device)
        else:
            x_tensor = res_x
        predicted_scores = []
        ...
        with torch.no_grad():
            for classifier in classifiers:
                predicted_scores.append(-1 * classifier(x_tensor))
    ...
    def paretoflow_sample(
        ...
        if (
            ...
            xl = task.xl
            xu = task.xu
            print(f"MIN original lower: {min(xl)} *** MAX original upper: {max(xu)}.")
            if task.is_discrete:
