# -*- coding: utf-8 -*-
#source: https://github.com/b-shields/edbo/blob/master/edbo/base_models.py
# Imports
...
class gp_model(ExactGP):
    ...
    def __init__(self, X, y, likelihood, gpu=True, nu=2.5,
                 lengthscale_prior=None, outputscale_prior=None, device = None
                 ):
        """        
        Parameters
        ----------
        ...
        if lengthscale_prior != None:
            try:
                ls_init = to_torch(lengthscale_prior[1], gpu=gpu, device = self.device)
                self.covar_module.base_kernel.lengthscale = ls_init
            except:
                uniform = to_torch(lengthscale_prior[1], gpu=gpu, device = self.device)
                ls_init = torch.ones(num_dims) * uniform
                self.covar_module.base_kernel.lengthscale = ls_init
            
        if outputscale_prior != None:
            os_init = to_torch(outputscale_prior[1], gpu=gpu, device = self.device)
