...
def to_torch(data, gpu=True, device = None):
    """
    ...
    if torch.cuda.is_available() and gpu == True and device is not None:
        torch_data = torch_data.to(device)
    
    return torch_data

def torch_to_numpy(data, gpu=True):
    """
...
def chunk_sample(model, domain_tensor, batch_size, gpu=True, chunk_size=5000):
    """
...
def sample(model, domain_tensor, batch_size, gpu=True, chunk_size=5000):
    """
    ...
    if len(domain_tensor) < chunk_size:
        ...
        samples = chunk_sample(model, domain_tensor, batch_size, gpu=True, chunk_size=chunk_size)
        
    return samples
    
# ArgMax a set of posterior draws
        
def join_to_df(sample, domain, gpu=True):
    """
