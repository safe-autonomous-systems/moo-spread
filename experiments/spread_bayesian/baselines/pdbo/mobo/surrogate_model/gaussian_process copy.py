...
class GaussianProcess(SurrogateModel):
    ...
    def evaluate(self, X, std=False, calc_gradient=False, calc_hessian=False):
        ...
        for gp in self.gps:
            ...
            #         # gp._K_inv = cho_solve((gp.L_, True), np.eye(gp.L_.shape[0]))
            ...
            if std:
                n_train = gp.L_.shape[0]

                # Recompute inverse when missing OR wrong size
                if (not hasattr(gp, "_K_inv")) or (gp._K_inv.shape[0] != n_train):
                    # Solve Lᵀ X = I to get L⁻ᵀ  (L is lower‐triangular)
                    L_inv = solve_triangular(gp.L_.T,
                                            np.eye(n_train),
                                            lower=False)             # solve_triangular 
                    # Form K⁻¹ = L⁻ᵀ·L⁻¹
                    gp._K_inv = L_inv.dot(L_inv.T)              # correct shape (n_train, n_train)

                # Now safe to compute variances:
                y_var = gp.kernel_.diag(X)
                correction = np.einsum("ij,ij->i",
                                    np.dot(K, gp._K_inv),    
                                    K)
                y_var -= correction
                y_var = np.where(y_var < 0, 0.0, y_var)
                y_std = np.sqrt(y_var)
                S.append(y_std) # y_std: shape (N,)
