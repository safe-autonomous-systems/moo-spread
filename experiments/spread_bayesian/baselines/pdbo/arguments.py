...
def get_general_args(args=None):
    ...
    parser.add_argument('--n-init-sample', type=int, 
                        default=100, help='number of initial design samples')
    parser.add_argument('--batch-size', type=int, 
                        default=5, help='size of the selected batch in one iteration')
    parser.add_argument('--seed', type=int, default=0, help='random seed')
    parser.add_argument('--n-seed', type=int, default=1, help='number of random seeds / test runs')
    parser.add_argument('--algo', type=str, 
                        default="PDBO", help='type of MOO algorithm to use')
...
def get_solver_args(args=None):
    ...
    parser.add_argument('--n-gen', type=int, default=25, help='number of generations')
    parser.add_argument('--pop-init-method', type=str, choices=['nds', 'random', 'lhs'], default='nds', help='method to init population')
    parser.add_argument('--n-process', type=int, default=cpu_count(), help='number of parallelized processes')
    parser.add_argument('--batch-size', type=int, default=5, help='size of the selected batch in one iteration')
    args, _ = parser.parse_known_args(args)
    return args
