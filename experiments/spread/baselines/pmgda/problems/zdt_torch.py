...
######## ZDT Problems ########
# For the sake of differentiability, we will use the strict bounds:
# - Lower bound: 0 + 1e-6 (instead of 0)
# - Upper bound: 1 - 1e-6 (instead of 1)

class BaseZDT(PymooProblemTorch):
    """
...
class ZDT1(BaseZDT):
    """
    ZDT1 test problem on PyTorch, fully differentiable.
    """
...
class ZDT2(BaseZDT):
    """
    ZDT2 test problem on PyTorch, fully differentiable.
    """
    def _calc_pareto_front(self, n_pareto_points: int = 100) -> torch.Tensor:
        x = torch.linspace(0.0, 1.0, n_pareto_points, device=self.device)
...
class ZDT3(BaseZDT):
    """
    ZDT3 test problem on PyTorch, fully differentiable.
    """
