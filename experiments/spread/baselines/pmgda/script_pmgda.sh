...
# Use command: chmod u+x script_pmgda.sh
...
for problem in "${problems[@]}"; do
    ...
    for seed in 1000 2000 3000 4000 5000; do
        export seed
        python3 run_pmgda.py --method="${method}" --problem="${problem}" --mode="solve" --seed="${seed}"
    done
    # Finally, run the report command (in the foreground)
    python3 run_pmgda.py --method="${method}" --problem="${problem}" --mode="report"
done